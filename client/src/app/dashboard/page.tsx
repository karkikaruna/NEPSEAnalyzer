...
export default function DashboardPage() {
  ...
  const [nepseData,  setNepseData]  = useState<any[]>([]);
  ...
  const load = useCallback(async()=>{
    setLoading(true);
    try {
      const [sR,gR,lR,syR,nR] = await Promise.all([
        ...
        fetch('/nepse/nepse-index?days=60').then(r=>r.json()),
      ]);
      setSummary(sR.summary??null); setTradingDate(sR.trading_date??'');
      setGainers(gR.data??[]); setLosers(lR.data??[]);
      setSymbols(syR.symbols??[]); setNepseData(nR.data??[]);
  ...
  const addToWatchlist = async()=>{
    ...
    try {
      const res = await fetch('/nepse/watchlist',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({symbol:selSymbol})});
      const j   = await res.json();
      if(!res.ok){setAddErr(j.detail??'Error');return;}
      setAddMsg(`${selSymbol} added to watchlist!`); setSelSymbol(''); setSymSearch('');
      setTimeout(()=>setAddMsg(''),3000);
    } finally { setAdding(false); }
  };

  // SVG market trend chart
  const renderTrend = ()=>{
    if(nepseData.length<2) return <div style={{height:100,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,color:'#6b7280'}}>Run load_history.py to see trend chart</div>;
    const W=560,H=100,PL=10,PR=55,PT=8,PB=24;
    const vals=nepseData.map(d=>Number(d.avg_close));
    const minV=Math.min(...vals)*0.997,maxV=Math.max(...vals)*1.003,range=maxV-minV||1;
    const cW=W-PL-PR,cH=H-PT-PB;
    const xs=nepseData.map((_,i)=>PL+i/(nepseData.length-1)*cW);
      ...
      <div>
        <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:8}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:4,padding:'3px 8px',borderRadius:6,background:last>=first?'#dcfce7':'#fee2e2',color:lc,fontSize:12,fontWeight:600}}>
            {last>=first?<ArrowUpRight size={12}/>:<ArrowDownRight size={12}/>}{Number(chg)>=0?'+':''}{chg}%
          </div>
          <div style={{fontSize:12,color:'#6b7280'}}>60-day avg close</div>
        </div>
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{display:'block'}}>
          {[0,0.25,0.5,0.75,1].map((_,i)=>{
            const v=minV+range*i/4; const y=PT+cH-(v-minV)/range*cH;
            return <g key={i}><line x1={PL} y1={y} x2={W-PR} y2={y} stroke="#f3f4f6" strokeWidth="1"/>
              <text x={W-PR+4} y={y+3.5} fontSize="9" fill="#9ca3af" fontFamily="Inter,system-ui">{v>=1000?`${(v/1000).toFixed(1)}k`:v.toFixed(0)}</text></g>;
          })}
          <defs><linearGradient id="grad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={lc} stopOpacity="0.15"/><stop offset="100%" stopColor={lc} stopOpacity="0.01"/></linearGradient></defs>
          ...
          {nepseData.filter((_,i)=>i%10===0||i===nepseData.length-1).map((d,_,arr)=>{
            const i=nepseData.indexOf(d); const dt=new Date(d.trading_date);
      ...
      <main style={{marginLeft:240,flex:1,padding:'24px 28px',overflowY:'auto'}}>
        ...
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:24}}>
          <div>
            ...
            <div style={{fontSize:13,color:'#6b7280',display:'flex',alignItems:'center',gap:8}}>
              ...
              Live NEPSE data · {tradingDate||'Loading…'}
            </div>
          </div>
        ...
        <div style={{background:'#fff',borderRadius:12,border:'1px solid #e5e7eb',padding:'18px 20px',marginBottom:20}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}>
            ...
            <span style={{fontSize:11,color:'#9ca3af',marginLeft:'auto'}}>Average close · last 60 trading days</span>
          </div>
        ...
        <div style={{background:'#fff',borderRadius:12,border:'1px solid #e5e7eb',padding:'18px 20px',marginBottom:20}}>
          <div style={{fontSize:14,fontWeight:600,color:'#111928',marginBottom:12}}>Add to Watchlist</div>
          <div style={{display:'flex',gap:10,alignItems:'flex-end',flexWrap:'wrap'}}>
            ...
            <button onClick={addToWatchlist} disabled={!selSymbol||adding}
              style={{display:'flex',alignItems:'center',gap:6,padding:'9px 18px',borderRadius:8,border:'none',cursor:!selSymbol||adding?'not-allowed':'pointer',background:'#1a56db',color:'#fff',fontWeight:600,fontSize:13,opacity:!selSymbol||adding?.6:1,fontFamily:'inherit'}}>
